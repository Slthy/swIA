...
export function Logo({ compact = false, inverse = false }: { compact?: boolean; inverse?: boolean }) {
  return (
    <div className="flex min-w-0 items-center gap-3" aria-label="GW SwimTrack">
      <span className={cn("grid size-10 shrink-0 place-items-center rounded-xl", inverse ? "bg-white/10" : "bg-[#0a304a]")}>
      ...
      {!compact && (
        <span className={cn("truncate whitespace-nowrap text-[1.05rem] font-bold tracking-[-0.02em]", inverse ? "text-white" : "text-[#0a304a]")}>
