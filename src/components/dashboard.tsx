...
export function Dashboard({ data, compact = false }: { data: DashboardData; compact?: boolean }) {
  const cards = [
    ...
    ["Best 25y", formatNumber(data.summary.best25ySeconds, 2), "seconds", Trophy, "#8d7448"],
  ] as const;
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2"><WellnessChart data={data.wellness} /><LoadChart data={data.load} /></div>
      {!compact && <RecoveryChart data={data.recovery} />}
      <HrZoneChart data={data.zones} />
      {!compact && <div className="grid gap-5 2xl:grid-cols-2"><SwimTestChart data={data.swimTests} /><FatigueChart data={data.fatigue} /></div>}
    </div>
  );
}
